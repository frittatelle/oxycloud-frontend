...
const ShareModal = ({ open, handleClose, shareParams, setShareParams, onComplete }) => {
  const [userMail, setUserMail] = useState("");
  const [ghostState, setGhostState] = useState(false);
        ...
        success: {
          render() {
            ...
            setShareParams(
              {
                ...shareParams,
                shared_with_mails: [...(shareParams.shared_with_mails || []), userMail]
              }
            );
            return `${shareParams.name} shared with ${userMail}`
          }
        },
        error: {
          render({ data }) {
        ...
        success: {
          render() {
            ...
            setShareParams(
              {
                ...shareParams,
                shared_with_mails: shareParams.shared_with_mails.filter(
                  (mail) => mail !== shareUserMail
                )
              }
            );
            return `${shareUserMail} removed from ${shareParams.name} share list `
          }
        },
        error: {
          render({ data }) {
